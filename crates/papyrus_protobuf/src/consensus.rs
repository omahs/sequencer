...
// TODO(guyn): Remove this when we clear up the conversion between transactions.
// This is the same as ProposalPart, only it uses Transaction, not ExecutableTransaction.
// This is used only in the PapyrusConsensusContext, which is used only for tests.
// Once we settle the issue of transaction types and conversions we should get rid of this.
#[derive(Debug, Clone, PartialEq)]
pub struct TemporaryTransactionBatch {
    /// The transactions in the batch.
    pub transactions: Vec<Transaction>,
}

#[derive(Debug, Clone, PartialEq)]
pub enum TemporaryProposalPart {
    /// The initialization part of the proposal.
    Init(ProposalInit),
    /// A part of the proposal that contains one or more transactions.
    Transactions(TemporaryTransactionBatch),
    /// The final part of the proposal, including the block hash.
    Fin(ProposalFin),
}

impl TryInto<ProposalInit> for TemporaryProposalPart {
    type Error = ProtobufConversionError;

    fn try_into(self: TemporaryProposalPart) -> Result<ProposalInit, Self::Error> {
        match self {
            TemporaryProposalPart::Init(init) => Ok(init),
            _ => Err(ProtobufConversionError::WrongEnumVariant {
                type_description: "ProposalPart",
                value_as_str: format!("{:?}", self),
                expected: "Init",
...
impl From<ProposalInit> for TemporaryProposalPart {
    fn from(value: ProposalInit) -> Self {
        TemporaryProposalPart::Init(value)
    }
}
